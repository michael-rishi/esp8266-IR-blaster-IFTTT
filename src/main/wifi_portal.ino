...
void wifi_setup() {
    ...
    if (SPIFFS.exists("/config.json")) {
      ...
      if (configFile) {
        ...
        if (json.success()) {
          ...
          strcpy(passcode, json["passcode"]);
          strcpy(port_str, json["port_str"]);
  ...
  WiFiManagerParameter custom_host_name("host_name", "host name", host_name, 20);
  WiFiManagerParameter custom_passcode("passcode", "passcode", passcode, 20);
  WiFiManagerParameter custom_port_str("port_str", "port string", port_str, 6);
  WiFiManagerParameter custom_user_id("user_id", "user id", user_id, 60);
  ...
  wifiManager.addParameter(&custom_port_str);
  ...
  strcpy(passcode, custom_passcode.getValue());
  strcpy(port_str, custom_port_str.getValue());
